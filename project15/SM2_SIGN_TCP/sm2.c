...
int sm3_e(uint8_t* userid, int userid_len, SM2_PUBKEY pk, uint8_t* msg, int msg_len, uint8_t* e)
{
	...
	*/
	...
	if ((pk.pubXLen > 32) || (pk.pubYLen > 32))
		return -1;
	...
	memcpy(buf + 2 + userid_len + 128 + 32 - pk.pubXLen, pk.pubX, 32);
	memcpy(buf + 2 + userid_len + 128 + 32 + 32 - pk.pubYLen, pk.pubY, 32);
...
sm2_sign_again:
	...
	if (rlen != NULL && slen != NULL) {
		*rlen = big_to_bytes(32, r, (char*)cr, TRUE);
		*slen = big_to_bytes(32, s, (char*)cs, TRUE);
	}
	else {
		big_to_bytes(32, r, (char*)cr, TRUE);
		big_to_bytes(32, s, (char*)cs, TRUE);
	}
...
int sm2_verify(uint8_t* hash, int hashlen, uint8_t* cr, int rlen, uint8_t* cs, int slen, SM2_PUBKEY pk)
{
	...
	*/
	...
	w = epoint_init();
	epoint_set(x, y, 0, g);
	bytes_to_big(pk.pubXLen, (char*)pk.pubX, x);
	bytes_to_big(pk.pubYLen, (char*)pk.pubY, y);
