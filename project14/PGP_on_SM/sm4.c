...
void sm4Enc(const uint8_t* inbuf, size_t dataLen, uint8_t* outbuf, const uint8_t* key) {
    ...
    if (dataLen % SM4_blockS) {
        printf("No Padding for SM4!\n");
        return;
    }

    int i;
#pragma omp parallel for
    for (i = 0; i < dataLen; i += SM4_blockS) {
        uint8_t* intemp = inbuf + i;
        uint8_t* outtemp = outbuf + i;
        IAE_SM4_encrypt(intemp, outtemp, &ks);
    }
}
...
#pragma omp parallel for
    for (i = 0; i < dataLen; i += SM4_blockS) {
        uint8_t* intemp = inbuf + i;
        uint8_t* outtemp = outbuf + i;
        IAE_SM4_decrypt(intemp, outtemp, &ks);
    }
}